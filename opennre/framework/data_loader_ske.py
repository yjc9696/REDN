...
class SentenceREDataset(data.Dataset):
    ...
    def __init__(self, path, rel2id, tokenizer, kwargs, sort=False, sort_reverse=False):
        ...
        self.kwargs = kwargs
        self.sort = sort
        vocab_file_path = "D:/RDEN/pretrained/chinese_L-12_H-768_A-12/vocab.txt"
        # self.bert_tokenizer = tokenization.FullTokenizer(vocab_file=vocab_file_path,
        #                                              do_lower_case=True)  # 初始化 bert_token 工具
        # Load the file
        self.data = []
        self.NA_id = -1
        for r in rel2id:
        ...
        for i, line in enumerate(f.readlines()):
            ...
            if len(line) > 0:
                ...
                if "postag" not in _d:
                    ...
                    _d["token"] = _d["postag"]
                if len(_d["token"]) > 512 or len(_d["token"]) < 1:
                    continue
                _tt = {}
                _tt["token"] = []
                for c in _d["token"]:
                    _tt["token"].append(c['word'])
                spo_list = _d["spo_list"]
                text = _d["text"]
                for dd in spo_list:
                    _dd = _tt
                    _dh = {}
                    _dh["name"] = dd["object"]
                    _dh["pos"] = self.find_pos(_d["token"], dd["object"])
                    _dt = {}
                    _dt["name"] = dd["subject"]
                    _dt["pos"] = self.find_pos(_d["token"], dd["subject"])
                    if _dh["pos"]==[-1,-1] or _dt["pos"]==[-1,-1]:
                        continue
                    _dd["relation"] = dd['predicate']
                    _dd["entity_list"] = [_dh, _dt]
                    _dd["entity_pair_list"] = [[0, 1, self.rel2id[dd['predicate']]]]
                    indexed_tokens, att_mask, new_index = self.tokenizer(_dd)
                    _dd["indexed_tokens"] = indexed_tokens
                    _dd["att_mask"] = att_mask
                    _dd["new_index"] = new_index
                    _dd["seq_len"] = indexed_tokens.shape[-1]
                    # pos_head = _dh['pos']
                    # pos_tail = _dt['pos']
                    # pos_head = [new_index.index(i) for i in pos_head]
                    # pos_tail = [new_index.index(i) for i in pos_tail]
                    if indexed_tokens.shape[-1] <= 512:
                        self.data.append(_dd)
                        sentense_idx[len(_dd["token"])][" ".join(_dd["token"])] = len(self.data) - 1
        if sort:
